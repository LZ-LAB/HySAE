...
class HySAE(BaseClass):
    ...
    def __init__(self, n_ent, n_rel, input_drop, hidden_drop, feature_drop, pad_mode, emb_dim, emb_dim1, max_arity, device, 
                 k_size, k_sizeN, dil_sizeN, lamda):
        ...
        self.k_size = k_size
        self.k_sizeN = k_sizeN
        
        self.dil_size = 1 # fixed
        self.dil_sizeN = dil_sizeN
        
        self.lamda = lamda
        
        self.kd_size = self.dil_size*(self.k_size - 1)
        self.kd_sizeN = self.dil_sizeN*(self.k_sizeN - 1)
        
        self.pad_size = self.kd_size // 2
        self.pad_sizeN = self.kd_sizeN // 2



        # if self.k_size % 2 == 1:
        #     self.conv_size = (self.emb_dim1 * self.emb_dim2) * 8 // 4
        # elif self.k_size % 2 == 0 and self.dil_sizeN % 2 == 0:
        #     self.conv_size = (self.emb_dim1 * self.emb_dim2) * 8 // 4
        # else:
        #     self.conv_size = ((self.emb_dim1 - 1) * (self.emb_dim2 - 1)) * 8 // 4

        self.conv_size = (self.emb_dim1 * self.emb_dim2) * 8 // 4
        self.conv_size2 = ((self.emb_dim1 - 1) * (self.emb_dim2 - 1)) * 8 // 4
        ...
        self.conv_layer_2N = nn.Conv3d(in_channels=1, out_channels=8, kernel_size=(self.k_sizeN, self.k_sizeN, 2), 
                                       padding=(self.pad_sizeN,self.pad_sizeN,0), padding_mode = self.pad_mode, dilation=(self.dil_sizeN,self.dil_sizeN,1))       
        self.conv_layer_3N = nn.Conv3d(in_channels=1, out_channels=8, kernel_size=(self.k_sizeN, self.k_sizeN, 3), 
                                       padding=(self.pad_sizeN,self.pad_sizeN,0), padding_mode = self.pad_mode, dilation=(self.dil_sizeN,self.dil_sizeN,1))
        self.conv_layer_4N = nn.Conv3d(in_channels=1, out_channels=8, kernel_size=(self.k_sizeN, self.k_sizeN, 4), 
                                       padding=(self.pad_sizeN,self.pad_sizeN,0), padding_mode = self.pad_mode, dilation=(self.dil_sizeN,self.dil_sizeN,1))
        self.conv_layer_5N = nn.Conv3d(in_channels=1, out_channels=8, kernel_size=(self.k_sizeN, self.k_sizeN, 5), 
                                       padding=(self.pad_sizeN,self.pad_sizeN,0), padding_mode = self.pad_mode, dilation=(self.dil_sizeN,self.dil_sizeN,1))
        self.conv_layer_6N = nn.Conv3d(in_channels=1, out_channels=8, kernel_size=(self.k_sizeN, self.k_sizeN, 6), 
                                       padding=(self.pad_sizeN,self.pad_sizeN,0), padding_mode = self.pad_mode, dilation=(self.dil_sizeN,self.dil_sizeN,1))
        self.conv_layer_7N = nn.Conv3d(in_channels=1, out_channels=8, kernel_size=(self.k_sizeN, self.k_sizeN, 7), 
                                       padding=(self.pad_sizeN,self.pad_sizeN,0), padding_mode = self.pad_mode, dilation=(self.dil_sizeN,self.dil_sizeN,1))
        self.conv_layer_8N = nn.Conv3d(in_channels=1, out_channels=8, kernel_size=(self.k_sizeN, self.k_sizeN, 8), 
                                       padding=(self.pad_sizeN,self.pad_sizeN,0), padding_mode = self.pad_mode, dilation=(self.dil_sizeN,self.dil_sizeN,1))
        self.conv_layer_9N = nn.Conv3d(in_channels=1, out_channels=8, kernel_size=(self.k_sizeN, self.k_sizeN, 9), 
                                       padding=(self.pad_sizeN,self.pad_sizeN,0), padding_mode = self.pad_mode, dilation=(self.dil_sizeN,self.dil_sizeN,1))   
        ...
        self.fc_layer2 = nn.Linear(in_features=self.conv_size2, out_features=self.emb_dim)
        
        self.W = nn.Linear(in_features=self.conv_size2, out_features=self.conv_size)
        ...
        nn.init.xavier_uniform_(self.conv_layer_2N.weight.data)
        nn.init.xavier_uniform_(self.conv_layer_3N.weight.data)
        nn.init.xavier_uniform_(self.conv_layer_4N.weight.data)
        nn.init.xavier_uniform_(self.conv_layer_5N.weight.data)
        nn.init.xavier_uniform_(self.conv_layer_6N.weight.data)
        nn.init.xavier_uniform_(self.conv_layer_7N.weight.data)
        nn.init.xavier_uniform_(self.conv_layer_8N.weight.data)
        nn.init.xavier_uniform_(self.conv_layer_9N.weight.data)
    ...
    def conv3d_dilated(self, concat_input):
        ...
        shape_size = x.shape
        result = np.prod(shape_size)
        
        if result % 800 == 0:
            x = x.view(-1, self.conv_size)
        else:
            x = x.view(-1, self.conv_size2)
    ...
    def conv3d_dilatedN(self, concat_input):
        r = concat_input[:, 0, :].view(-1, 1, self.emb_dim1, self.emb_dim2)
    ...
    def forward(self, rel_idx, ent_idx, miss_ent_domain):
        ...
        v2 = self.conv3d_dilatedN(concat_input)
 
        v1out_shape = np.prod(v1.shape)
        v2out_shape = np.prod(v2.shape)        
 
        if v1out_shape % 800 == 0 and v2out_shape % 800 == 0:
            v1out = v1
            v2out = v2
        elif v1out_shape % 800 != 0 and v2out_shape % 800 == 0:
            v1out = self.W(v1)
            v2out = v2
        elif v1out_shape % 800 == 0 and v2out_shape % 800 != 0:
            v1out = v1
        ...
        v_out = self.lamda*v1out + (1-self.lamda)*v2out
        v_out = self.hidden_drop(v_out) # hidden_drop


        if v1out_shape % 800 != 0 and v2out_shape % 800 != 0:
            x = self.fc_layer2(v_out)
        else:
