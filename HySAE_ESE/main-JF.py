...
from modelHySAEese import HySAE
...
class Experiment:
    def __init__(self, num_iterations, batch_size, lr, dr, input_drop, hidden_drop, feature_drop, pad_mode, dembd, dembd1, max_ary,
        ...
        self.k_size, self.k_sizeN, self.dil_sizeN = k_size, k_sizeN, dil_sizeN
        self.lamda = lamda
    ...
    def train_and_eval(self):
        ...
        model = HySAE(len(d.ent2id), len(d.rel2id), self.input_drop, self.hidden_drop, self.feature_drop, self.pad_mode,
                      self.dembd, self.dembd1, self.max_ary, self.device,
                      self.k_size, self.k_sizeN, self.dil_sizeN, self.lamda)
        model.to(device)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--lr", type=float, default=0.00050, nargs="?", help="Learning rate.")
    parser.add_argument("--dr", type=float, default=0.988, nargs="?", help="Decay rate.")
    
    parser.add_argument("--input_drop", type=float, default=0.8, nargs="?", help="input_drop.")
    parser.add_argument("--hidden_drop", type=float, default=0.2, nargs="?", help="hidden_drop.")
    parser.add_argument("--feature_drop", type=float, default=0.3, nargs="?", help="feature_drop.")
    
    parser.add_argument("--pad_mode", type=str, default='zeros', nargs="?", help="convolutional padding mode.")
    
    parser.add_argument("--k_size", type=int, default=4, nargs="?", help="convolutional kernel size.")
    parser.add_argument("--k_sizeN", type=int, default=3, nargs="?", help="convolutional kernel size N.") 
    parser.add_argument("--dil_sizeN", type=int, default=5, nargs="?", help="convolutional dilation size N.")
    
    parser.add_argument("--lamda", type=float, default=0.4, nargs="?", help="lamda.")

    
    parser.add_argument("--dembed", type=int, default=400, nargs="?")
    parser.add_argument("--dembed1", type=int, default=20, nargs="?")
    ...
    ### 2-ary dataset，WN18RR/FB15K-237
    # args.ary_list = [2]
    ...
    param['pad_mode'] = args.pad_mode
    
    param['k_size'], param['k_sizeN'], param['dil_sizeN'] = args.k_size, args.k_sizeN, args.dil_sizeN

    param['lamda'] = args.lamda
    ...
    Exp = Experiment(num_iterations=param['num_iterations'], batch_size=param['batch_size'], lr=param['lr'], dr=param['dr'],
                     ...
                     pad_mode=param['pad_mode'], dembd=param['dembed'], dembd1=param['dembed1'], max_ary=d.max_ary,
                     k_size=param['k_size'], k_sizeN=param['k_sizeN'], dil_sizeN=param['dil_sizeN'], lamda=param['lamda'])
    Exp.train_and_eval()
